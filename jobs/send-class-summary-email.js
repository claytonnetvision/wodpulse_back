...
// Contador de requests diário para Gemini (free tier: ~1500/dia, usamos margem de 1200)
let geminiRequestsHoje = 0;
let ultimoDia = new Date().toDateString();
const LIMITE_DIARIO_GEMINI = 1200; // margem segura (80% do free tier)

// Função auxiliar para reset diário
function resetGeminiContadorSeNecessario() {
  const hoje = new Date().toDateString();
  if (hoje !== ultimoDia) {
    console.log(`[GEMINI LIMIT] Reset diário: total requests ontem = ${geminiRequestsHoje}`);
    geminiRequestsHoje = 0;
    ultimoDia = hoje;
  }
}
...
async function sendSummaryEmailsAfterClass(sessionId) {
  try {
    ...
    for (const aluno of participantsRes.rows) {
      ...
      resetGeminiContadorSeNecessario();

      // === INTEGRAÇÃO GEMINI COM LIMITE DIÁRIO ===
      let comentarioIA = 'Cada treino soma. Mantenha o foco e os números vão subir cada vez mais! 💪'; // fallback

      if (geminiRequestsHoje >= LIMITE_DIARIO_GEMINI) {
        console.log(`[GEMINI LIMIT] Limite diário atingido (${geminiRequestsHoje}/${LIMITE_DIARIO_GEMINI}) - usando fallback para ${aluno.name}`);
      } else {
        geminiRequestsHoje++;
        console.log(`[GEMINI LIMIT] Requests hoje: ${geminiRequestsHoje}/${LIMITE_DIARIO_GEMINI} - avaliando ${aluno.name}`);
          ...
          const geminiResponse = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${process.env.GEMINI_API_KEY}`,
            {
              ...
              body: JSON.stringify({
                ...
                generationConfig: {
                  temperature: 0.7,
                  maxOutputTokens: 400  // aumentado para texto completo
                }
              })
            }
          );
